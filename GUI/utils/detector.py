...
# Edge detection
def edge_detection(image, normalise=True):
    if normalise:
        image = np.clip(image, 0, 1)
        image = (image * 255).astype(np.uint8)
    edges = cv2.Canny(image.astype('uint8'), 50, 150)
    return edges


# Hough transform
def __extract_lines(edges, threshold=10):
...
def __filter_lines_by_angle(lines, angle_min=-30, angle_max=-1):
    filtered_lines = []
    for line in lines:
...
def __filter_lines_by_y(lines, y_min):
    filtered_lines = []
    for line in lines:
...
# Calculate the slope and intercept of the lines
def __calculate_slope_intercept(lines):
    m_values = []
    c_values = []
    len(lines)
    for line in lines:
...
def visualise_line(im, slope, intercept):
    ...
    if max_value < 1:
        ...
        im = (im - min_value) / (max_value - min_value) * 255
...
# Perform the Hough transform to detect deep aponeuroses
def __hough_transform_deep(edges, angle_min=-30, angle_max=-1, visualise=None, threshold=20):
    lines = __extract_lines(edges, threshold)
    if lines is None:
        return edges, None, None
    filtered_lines = __filter_lines_by_angle(lines, angle_min, angle_max)
    if filtered_lines == []:
        # print("No lines found, reducing the angle range")
        return edges, None, None
    slope, intercept = __calculate_slope_intercept(filtered_lines)
    ...
    while True:
        filtered_lines = __filter_lines_by_angle(filtered_lines, angle - 2, angle + 2)
        new_slope, new_intercept = __calculate_slope_intercept(filtered_lines)
        if new_slope is None or new_intercept is None:
            break 
        new_angle = __slope_to_angle(new_slope)
...
# Perform the Hough transform to detect superficial aponeuroses
def __hough_transform_superficial(edges, y_max, angle_min=-10, angle_max=10, visualise=None, threshold=20):
    slope = 0
    intercept = 0
    lines = __extract_lines(edges, threshold)
    if lines is None:
    ...
    if not filtered_lines:  # Check if list is empty
        ...
        return edges, slope, intercept
    filtered_lines = __filter_lines_by_y(filtered_lines, y_max)
...
# Perform the Hough transform to detect both deep and superficial aponeuroses
def hough_transform(edges, angle_min_d=-30, angle_max_d=-1, angle_min_s=-10, angle_max_s=10, visualise=None, threshold=20):
    # Deep aponeuroses
    aponeuroses_line_d, slope_d, intercept_d = __hough_transform_deep(edges, angle_min_d, angle_max_d, visualise, threshold)
    ...
    aponeuroses_line_s_d, slope_s, intercept_s = __hough_transform_superficial(edges, y_max, angle_min_s, angle_max_s, aponeuroses_line_d, threshold)
    if slope_s == None:
        aponeuroses_line_s_d = aponeuroses_line_d
